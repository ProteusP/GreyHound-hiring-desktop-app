...
void Auth::login(const HttpRequestPtr &req,
    ...
    LOG_DEBUG << "Executing SQL query: " << sqlString
              << " with email: " << email << "\n";
    ...
    dbClient->execSqlAsync(
        ...
        [=](const orm::Result &result) {
            ...
            try {
                ...
                LOG_DEBUG << "Got a session: User id: "
...
void Auth::registerUser(
    ...
    if (!jsonReq || !jsonReq->isMember("status") ||
        (*jsonReq)["status"].asString().empty()) {
        Json::Value json;
    ...
    orm::Criteria findUserCriteria{
        drogon_model::default_db::Users::Cols::_email, orm::CompareOperator::EQ,
        email};
...
void Auth::logout(const HttpRequestPtr &req,
    ...
    try {
        redisClientPtr->execCommandAsync(
            [](const nosql::RedisResult &r) {
                LOG_DEBUG << "Result of deleting cache: "
    ...
    resp->addHeader("Set-Cookie",
                    "JSESSIONID=deleted; Path=/; HttpOnly; Max-Age=0");
    callback(resp);
}
