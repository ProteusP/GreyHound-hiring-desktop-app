...
void resources::getVacancies(
    ...
    try {
        const auto &dbClient = app().getDbClient();
        auto mapper =
            orm::Mapper<drogon_model::default_db::Vacancies>(dbClient);
...
void resources::getResume(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback,
    const std::string &userId) {

    const std::string filePath =
        app().getUploadPath() + "/" + makeResumePath(userId);
    ...
    resp->addHeader("Content-Disposition",
                    "inline; filename=\"" + userId + "_resume.pdf\"");
    resp->setBody(content);
    callback(resp);
}

void resources::saveResume(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback) {
    ...
    if (const auto &ext = fileParser.getFiles()[0].getFileExtension();
        ext != "pdf") {
        const auto resp = HttpResponse::newHttpResponse();
        resp->setStatusCode(HttpStatusCode::k400BadRequest);
        resp->setContentTypeCode(CT_TEXT_PLAIN);
...
void resources::getCandidateCards(
    ...
    const auto &experienceStatusIdFilter =
        req->getParameter("experience_status_id");
    const auto &workScheduleIdFilter =
        req->getParameter("work_schedule_status_id");

    const int offset = (page - 1) * pageSize;
    const auto &dbClient = app().getDbClient();

    std::string sql = "SELECT user_id, place_of_study, faculty_of_educ, "
                      "experience_status_id FROM candidates WHERE 1=1";
    ...
    sql += " LIMIT " + std::to_string(pageSize) + " OFFSET " +
    ...
    binder >> [callback, dbClient, page,
               pageSize](const drogon::orm::Result &candidatesResult) mutable {
        ...
        dbClient->execSqlAsync(
            expSql,
            [callback, candidatesResult, candidateIds, page,
             pageSize](const drogon::orm::Result &expResult) {
                std::unordered_map<int, std::string> candidateExp;
                for (const auto &row : expResult) {
                    candidateExp[row["user_id"].as<int>()] =
...
void resources::deleteVacancy(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback, const int vacId) {
    const auto &dbClient = app().getDbClient();
    auto vacMapper = orm::Mapper<drogon_model::default_db::Vacancies>(dbClient);
    const std::string &vacIdStr = std::to_string(vacId);

    const int countOfDeleted = vacMapper.deleteBy(
        orm::Criteria(drogon_model::default_db::Vacancies::Cols::_id,
                      orm::CompareOperator::EQ, vacIdStr));
...
void resources::getEmplVacancies(
    ...
    auto vacCriteria =
        orm::Criteria(drogon_model::default_db::Vacancies::Cols::_employer_id,
...
void resources::updateVacancy(
    ...
    try {
        ...
        const auto criteria =
            orm::Criteria(drogon_model::default_db::Vacancies::Cols::_id,
                          orm::CompareOperator::EQ, std::to_string(vacId));
...
void resources::getVacanciesCards(
    ...
    const auto &experienceStatusIdFilter =
        req->getParameter("experience_status_id");
    const auto &workScheduleStatusIdFilter =
        req->getParameter("work_schedule_status_id");
    const auto &educStatusIdFilter = req->getParameter("educ_status_id");
    const auto &remotenessStatusIdFilter =
        req->getParameter("remoteness_status_id");

    const int offset = (page - 1) * pageSize;
    const auto &dbClient = app().getDbClient();

    std::string sql =
        "SELECT id, name, salary, place, work_schedule_status_id, employer_id, "
        "remoteness_status_id, status FROM vacancies WHERE 1=1";
    ...
    binder >> [callback, dbClient, page,
               pageSize](const orm::Result &vacanciesResult) mutable {
        std::set<int> emplIds;
        ...
        const std::string companySql =
            "SELECT user_id, company_name FROM employers WHERE user_id IN (" +
            placeholders + ")";

        dbClient->execSqlAsync(
            companySql,
            [callback, vacanciesResult, page,
             pageSize](const orm::Result &companiesResult) mutable {
                std::unordered_map<int, std::string> companies;
                for (const auto &row : companiesResult)
                    companies[row["user_id"].as<int>()] =
                ...
                for (const auto &row : vacanciesResult) {
                    ...
                    vacJson["work_schedule_status_id"] =
                        row["work_schedule_status_id"].as<std::string>();
                    vacJson["remoteness_status_id"] =
                        row["remoteness_status_id"].as<std::string>();
...
void resources::getCandidateInfo(
    ...
    const std::string sql =
        "SELECT name, surname, email, phone_num, place, place_of_study, "
        "faculty_of_educ, experience_status_id FROM candidates WHERE user_id = "
        "?";
    dbClient->execSqlAsync(
        sql,
        [callback, &userId](const orm::Result &candidate) {
            ...
            jsonResp["study_info"] = row["place_of_study"].as<std::string>() +
                                     " " +
                                     row["faculty_of_educ"].as<std::string>();
            jsonResp["experience_status_id"] =
                row["experience_status_id"].as<int>();
